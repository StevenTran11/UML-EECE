...
def testbench(dut):
	...
	impl = get_bits(features, (31, 24))
	dut._log.info(f'features:     {features:08x}')
	dut._log.info(f'    impl:     {impl:02x}')
	if os.path.exists('implementers.cfg'):
		from configparser import ConfigParser
		cfg = ConfigParser()
		cfg.read('implementers.cfg')
		if f'{impl}' in cfg['vendors'].keys():
			dut._log.info(f'              ({cfg["vendors"][f"{impl}"]})')
		else:
			dut._log.info(f'              (unknown)')
	else:
		dut._log.warning(f'              (no implementers file found)')
